-- ============================================================
-- SCRIPT CHUẨN 100%: TẠO BẢNG & NẠP DỮ LIỆU CẦU THỦ THÁNG 3 -> THÁNG 7
-- ============================================================

-- BƯỚC 1: Tạo bảng `players` nếu chưa có
...
-- BƯỚC 2: Tạo bảng `player_scores` nếu chưa có
...
-- BƯỚC 3: Tắt RLS để đọc/ghi không bị chặn phân quyền
ALTER TABLE public.players DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.player_scores DISABLE ROW LEVEL SECURITY;

-- BƯỚC 4: Xóa sạch dữ liệu cũ để nạp mới không bị trùng lặp
TRUNCATE TABLE public.player_scores CASCADE;
TRUNCATE TABLE public.players CASCADE;

-- BƯỚC 5: Nạp danh sách 41 Cầu thủ vào bảng `players`
...
-- BƯỚC 6: Nạp Bàn thắng & Kiến tạo Tháng 3 -> Tháng 7 vào bảng `player_scores`
